...
from src.views.auth_view import exigir_login_google, render_usuario_topbar
from src.views.cadastros_view import render_cadastros
from src.views.components.ui_components import aplicar_estilo_global, render_marca_sidebar
...
def main():
    ...
    if usuario_atual.pode("estoque.visualizar"):
        rotas_possiveis["▤  Estoque"] = render_estoque
    ...
    if usuario_atual.pode("producao.gerenciar"):
        rotas_possiveis["⚙  PCP e Produção"] = lambda: render_producao(usuario_atual)
    if usuario_atual.pode("vendas.gerenciar"):
        rotas_possiveis["＋  Novo pedido"] = lambda: render_vendas(usuario_atual)
    ...
    if usuario_atual.pode("logistica.gerenciar"):
        rotas_possiveis["▣  Logística"] = render_logistica
    ...
    pagina_selecionada = st.sidebar.radio(
        "Navegar para", list(rotas_possiveis), label_visibility="collapsed"
    )
    st.sidebar.markdown(
        '<div class="fn-version">FioNobre ERP · 2026.1</div>',
        unsafe_allow_html=True,
    )
    render_usuario_topbar(usuario_atual)
